...
export const TransactionFormModal: React.FC<TransactionFormModalProps> = ({ isOpen, onClose }) => {
  const { categories, banks, addTransaction, addCategory, addBank } = useApp();
      ...
      <Card className="w-full max-w-xl p-6 md:p-8 border-github-border shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto rounded-t-[2.5rem] md:rounded-[2.5rem]">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-2xl font-black text-github-text tracking-tighter uppercase">Novo Lançamento</h3>
          </div>
        ...
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div className="flex gap-4 pt-4">
            <Button onClick={onClose} variant="secondary" className="flex-1 py-4 text-lg rounded-2xl">Cancelar</Button>
            <Button type="submit" variant="primary" className="flex-1 py-4 text-lg rounded-2xl shadow-xl shadow-github-success/10 border border-github-border">
